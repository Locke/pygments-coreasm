...
class CoreASMLexer(RegexLexer):
    ...
    tokens = {
        ...
        'root': [
            ...
            (r'(<<|>>)', Punctuation),
            (r'(!=|<=|>=|=|[-<>+*/%|])', Operator),
            (r'(endchoose)\b', Keyword),
            (r'(end)?(seqblock|par|case|iterate|forall|foreach|if)\b', Keyword),
            (r'(seq|next|return|result)\b', Keyword),
            (r'(import|do|skip|pick|exists|in|is|with|holds|ifnone|then|else|while|step)\b', Keyword),
            ...
            (r'(local|let|choose)(\s+)(\w*)', bygroups(Keyword.Declaration, Text, Name.Variable.Instance)),
